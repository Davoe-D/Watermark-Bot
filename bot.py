...
async def HelpWatermark(bot, cmd):
	...
	await cmd.reply_text(
		...
		parse_mode="Markdown",
		reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Editor", url="https://t.me/BestzBotupdate"), InlineKeyboardButton("Support Group", url="https://t.me/naija_bestz")], [InlineKeyboardButton("Bots Channel", url="https://t.me/BestzBotupdate")], [InlineKeyboardButton("Source Code", url="https://github.com/Davoe-D/Watermark-Bot")]]),
...
async def VidWatermarkAdder(bot, cmd):
	...
	output_vid = main_file_name + "_[" + str(cmd.from_user.id) + "]_[" + str(time.time()) + "]_[@BestzBrothers]" + ".mp4"
	...
	if (int(file_size) > 2097152000) and (Config.ALLOW_UPLOAD_TO_STREAMTAPE is True) and (Config.STREAMTAPE_API_USERNAME != "NoNeed") and (Config.STREAMTAPE_API_PASS != "NoNeed"):
		...
		try:
			...
			print(f"Error: {e}")
			await editable.edit("Sorry, Something went wrong!\n\nCan't Upload to Streamtape. You can report at [Support Group](https://t.me/naija_bestz).")
...
async def button(bot, cmd: CallbackQuery):
	...
	if "refreshmeh" in cb_data:
		...
		await cmd.message.edit(
			text=Config.USAGE_WATERMARK_ADDER,
			parse_mode="Markdown",
			reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Editor", url="https://t.me/BestzBrothers"), InlineKeyboardButton("Support Group", url="https://t.me/naija_bestz")], [InlineKeyboardButton("Bots Channel", url="https://t.me/BestzBotupdate")]]),
			disable_web_page_preview=True
		)
